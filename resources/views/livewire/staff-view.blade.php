        ...
        <header class="fixed top-0 z-30 left-0 right-0 bg-white/90 backdrop-blur border-b border-gray-200 {{ $sidebarOpen ? 'lg:ml-64 lg:w-[calc(100%-16rem)]' : '' }}">
            <div class="h-14 flex items-center px-3">
                ...
                <div class="ml-auto flex items-center gap-3">
                    ...
                    {{-- SUPPORT INBOX BUTTON (Livewire with red dot) --}}
                    @if (auth()->check() && auth()->user()->identifier != 1)
                        @livewire('staff-support-inbox-button')
                    @endif
                    ...
                    <div class="bg-white rounded-2xl shadow p-6">
                        ...
                        <div class="px-6 py-5 border-b border-gray-200">
                            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                                ...
                                <div class="flex flex-wrap gap-2" wire:poll.10s="updateTabCounts">
                                    @php 
                                        $tabs = [
                                            'id'        => 'ID Apps',
                                            'renewal'   => 'Renewal',
                                    ...
                                    @foreach($tabs as $key => $label)
                                        @php
                                            $currentLatest = $tabLatestIds[$key] ?? 0;
                                            $seenLatest    = $tabSeenLatestIds[$key] ?? 0;
                                            $hasNew        = $currentLatest > 0 && $currentLatest > $seenLatest;
                                        @endphp

                                        <button 
                                            type="button" 
                                            wire:click="selectAppTab('{{ $key }}')"
